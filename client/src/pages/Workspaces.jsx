...
export const Workspaces = () => {
  ...
  const handleEnterWorkspace = (org) => {
    ...
    const rolePath = user?.role === 'SUPER_ADMIN' ? '/superadmin' : user?.role === 'ADMIN' ? '/admin' : '/member';
    navigate(`${rolePath}/team?workspaceId=${org.id}`);
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-2 border-b border-zinc-200/80">
        <div>
          <div className="flex items-center gap-2 text-xs font-mono text-zinc-500 mb-1">
            <span className="px-2.5 py-1 rounded bg-zinc-900 text-white font-semibold tracking-wide text-[11px] uppercase">
              WORKSPACES DIRECTORY
            </span>
            <span className="text-zinc-400">•</span>
            <span className="text-zinc-600 font-medium">Enterprise Organizations Governance</span>
          </div>
      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3">
        ...
        <div className="relative flex-1 min-w-[260px] max-w-md">
          ...
          <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
            {searchQuery ? (
              <button onClick={() => setSearchQuery('')} className="text-zinc-400 hover:text-zinc-600 p-0.5 rounded cursor-pointer">
